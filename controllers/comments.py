...
@router.get("/teas/{tea_id}/comments/{comment_id}", response_model=CommentSchema)
def get_single_comment(tea_id: int, comment_id: int, db: Session = Depends(get_db)):
    comment = db.query(CommentModel).filter(CommentModel.id == comment_id, CommentModel.tea_id == tea_id).first()
    if not comment:
